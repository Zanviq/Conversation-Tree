...
import { Send, GitFork, User, Sparkles, Pencil, Copy, X, Check, Paperclip, Menu, GitMerge, Info, Maximize2 } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { getThreadFromHead } from '../utils/graphUtils';

interface ChatInterfaceProps {
  messages: Message[];
  messageMap?: Record<string, Message>; // Added messageMap to lookup tracks
  ...
  // New: Delegate view track to parent to handle modal layering
...
// Track Colors matching Map (Should be shared const ideally)
const TRACK_COLORS = [
    'border-nebula-500 text-nebula-400 bg-nebula-950', // A
    'border-cyan-500 text-cyan-400 bg-cyan-950', // B
    'border-amber-500 text-amber-400 bg-amber-950', // C
    'border-pink-500 text-pink-400 bg-pink-950', // D
    'border-emerald-500 text-emerald-400 bg-emerald-950', // E
    'border-red-500 text-red-400 bg-red-950', // F
];
        ...
        {pairs.map((pair, index) => {
            ...
            return (
                <div key={pair.userMsg.id} className="relative group hover:z-30 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {/* Action Bar - FIX: top-full with padding instead of negative bottom margin to ensure hover continuity */}
