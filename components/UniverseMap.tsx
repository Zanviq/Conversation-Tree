...
const UniverseMap: React.FC<UniverseMapProps> = ({ 
  ...
  useEffect(() => {
    ...
    const links = hierarchyLayer.selectAll<SVGPathElement, d3.HierarchyPointLink<TreeNode>>(".link")
    ...
    const connGroup = connectionsLayer.selectAll<SVGGElement, any>(".connection-group")
    ...
    // 5. Draw Nodes
    const nodesSelection = nodesLayer.selectAll<SVGGElement, d3.HierarchyPointNode<TreeNode>>(".node")
    ...
    const drag = d3.drag<SVGGElement, d3.HierarchyPointNode<TreeNode>>()
        ...
        .on("drag", function(event, d) {
            ...
            if ((d as any)._isDragging) {
                ...
                hierarchyLayer.selectAll<SVGPathElement, d3.HierarchyPointLink<TreeNode>>(".link").attr("d", hierarchyLinkGenerator);
                nodePositionsRef.current[d.data.id] = { x: d.x, y: d.y };

                connectionsLayer.selectAll<SVGGElement, any>(".connection-group")
                    .each(function(linkD) {
                        const sel = d3.select(this);
        ...
        .on("end", function(event, d) {
            ...
            if (!(d as any)._isDragging) {
                 // Forward the check logic if we want, or rely on handleNodeClick within the click handler we attached above.
                 // Actually d3 drag suppresses click event sometimes if moved. 
                 // We handled click separately via .on("click").
            }
  ...
  useEffect(() => {
     ...
     const curve = d3.linkVertical<any, any>().source(d => d.source).target(d => d.target).x(d => d.x).y(d => d.y);
